  ...
  return (
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center gap-2">
            <AlertTriangle className={`h-4 w-4 ${validation.errors.length > 0 ? 'text-destructive' : validation.warnings.length > 0 ? 'text-primary' : 'text-success'}`} />
          ...
          <div className="text-2xl font-bold mt-1">
            {validation.errors.length + validation.warnings.length === 0 ? (
              ...
              <span className={validation.errors.length > 0 ? 'text-destructive' : 'text-primary'}>
